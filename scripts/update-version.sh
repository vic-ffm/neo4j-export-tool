...
echo "Files updated:"
echo "  - .version"
echo "  - Directory.Build.props (applies to all projects in solution)" 
echo "  - README.md"
echo ""
echo "The version will be automatically used in:"
echo "  - All F# projects (via Directory.Build.props inheritance)"
echo "  - Makefile (via .version file)"
echo "  - Docker compose (via VERSION env var)"
echo ""
echo "Projects affected:"
find "$PROJECT_ROOT" -name "*.fsproj" -type f | while read -r project; do
    echo "  - $(basename "$project" .fsproj)"
done